              ...
              <tbody class="text-center">
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <form method="POST" action="{{ route('project.destroy', [$type_project->slug, $item->id]) }}" class="d-inline">
                      @csrf
                      {{ method_field('delete') }}
                      <button type="submit" class="btn btn-danger btn-xs has-icon"><i class="mdi mdi-delete mr-0"></i></button>
                    </form>
                    @if (auth()->user()->roles == 'DIREKTUR')
                    <form method="POST" action="{{ route('project.status', [$type_project->slug, $item->id]) }}" class="d-inline">
                      @csrf
                      {{ method_field('put') }}
                      <button type="submit" class="btn btn-warning btn-xs has-icon"><i class="mdi mdi-check mr-0"></i></button>
                    </form>
                    @endif
                  </td>
                </tr>
